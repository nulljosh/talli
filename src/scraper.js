...
async function getLaunchOptions({ headless = true } = {}) {
 if (IS_VERCEL) {
 let chromium;
 try {
 chromium = require('@sparticuz/chromium');
 } catch (error) {
 throw new Error('Puppeteer auth unavailable on Vercel: @sparticuz/chromium is not installed');
 }
...
async function getAuthenticatedCookies(options = {}) {
 const { username, password, headless = true } = options;
 let browser;

 try {
 const launchOptions = await getLaunchOptions({ headless });
 ...
 loginSuccess = await attemptLogin(page, attempt, { username, password });
 ...
 const smSession = cookies.find(cookie => cookie.name === 'SMSESSION' && cookie.value);

 if (!smSession) {
 throw new Error('SMSESSION cookie not found after Puppeteer login');
 }
 ...
 if (error && /Could not find Chrome|Browser was not found|executablePath/i.test(error.message)) {
 throw new Error(`Puppeteer auth unavailable: ${error.message}`);
 }
 throw error;
 } finally {
 if (browser) {
 try { await browser.close(); } catch (closeError) { /* ensure close */ }
 }
 }
}
...
async function checkAllSections(options = {}) {
 ...
 console.log(` [x] ${data.allText.length} items found`);
...
module.exports = { checkAllSections, runSubmitMonthlyReport, submitMonthlyReport, getAuthenticatedCookies };

...
## Changelog
- v3.7.0 (2026-05-22): Blob legacy path fallback restored in loadUserBlob() — results.json for existing accounts was stored under pre-HMAC path (tally-cache/{userId}/results.json) and never migrated; app was falling through all caches to null → $0.00. Auto-migrates on first hit. Broadened payment amount regex across api.js + unified.html to catch label variants beyond "Amount:" (cheque, monthly, next payment, standalone $X,XXX). iOS $0.00 silent fallback changed to "--". Debug endpoint /api/debug-payment added.

...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      console.error('[RDSP] GET error:', err.message);
      return res.status(200).json({ status: 'pending', accountOpenedDate: null, accountNumber: null, notes: '' });
    }
  }
  ...
  if (req.method === 'POST') {
    try {
      ...
      console.error('[RDSP] POST error:', err.message);
      return res.status(500).json({ error: 'Failed to save RDSP profile' });
    }
  }

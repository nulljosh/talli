...
final class QuizViewModel {
    ...
    private var bank: [String: [Question]] {
        guard let data = quizData else { return [:] }
        return subject == .math ? data.math : data.science
    }

    var availableUnits: [Int] {
        subject.units.filter { bank[String($0)] != nil }
    }
    ...
    func start() {
        ...
        questionOptions = questions.map { buildOptions(for: $0) }
        idx = 0; selected = nil; locked = false; score = 0; done = false; missed = []
    }

    private func buildOptions(for q: Question) -> [String] {
        var opts = [q.a]
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                ...
                VStack(spacing: 10) {
                    ForEach(vm.questionOptions[vm.idx], id: \.self) { opt in

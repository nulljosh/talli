...
def get_creds():
    """Get D2L credentials from macOS Keychain, falling back to env vars."""
...
def parse_grade_table(page):
    ...
    if not table:
        # Fallback: d2l-table class
        table = soup.find('table', class_='d2l-table')
    if not table:
        ...
        for t in soup.find_all('table'):
            ...
            if th and 'grade' in th.get_text(strip=True).lower():
                ...
                break
    if not table:
        # Last resort: first table on page
...
def get_grades_for_course(page, course_name, course_ou):
    ...
    try:
        ...
        print(f"  WARNING: Failed to parse grade table: {e}")
        categories = []
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if not username or not password:
                print("ERROR: No credentials. Set D2L_USERNAME/D2L_PASSWORD or add to Keychain (service: d2l-langley).")
                return
            ...
            out_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "grades.json")

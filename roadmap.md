...
- [ ] **Talli needs a real `.env`** (only `.env.example` exists) with actual BC Self-Serve test credentials — Joshua's ask, for future screenshot/testing automation to log into a real account state instead of capturing the blank/login screen (which is what the Mac screenshot above actually shows — cosmetic fix later, replace with an authenticated dashboard screenshot once `.env` exists). Needs Joshua to provide real credentials; can't fabricate.

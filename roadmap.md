...
- [x] Next-payment card: grey line overlapping behind the amount/value — FIXED 2026-08-03 in the simulator. Root cause: `paymentProgress` always rendered, and when `appState.daysUntilPayment` is nil the `?? 28` fallback makes `pct` exactly 0 — so it drew an empty grey track with the dot pinned at the far-left edge, directly under the amount, conveying nothing. Now only rendered when `daysUntilPayment != nil` (`ContentView.swift:317`). Verified by screenshot before/after.
- [ ] Large unused whitespace at bottom of payment card view — **investigated in the sim 2026-08-03, no removable padding found.** Measured at max scroll: content ends ~63pt above the floating tab bar, and that gap is the clearance `.safeAreaPadding(.bottom, 90)` (`ContentView.swift:326`) reserves so content isn't hidden under `TalliFloatingTabBar`. The bar occupies ~94pt from the screen bottom, so if anything the 90 is ~4pt *short*, not excessive — reducing it would push content under the bar. The perceived emptiness is inherent to a floating tab bar over a scroll view that simply runs out of content. Closing this properly means a design decision (e.g. more content, or a non-floating bar), not a padding tweak. Don't re-investigate blind.

...
- [ ] Messages pagination past page 1 — detected (`hasMoreMessages()`, `has_more_messages`) but not implemented. `src/http-scraper.js` does a single GET of `/Auth/Messages`; needs a live signed-in portal session to discover the "Show More Messages" postback (likely ASP.NET `__doPostBack` + `__VIEWSTATE` — capture the Messages page HTML while signed in, find the control's `name`, POST the hidden fields back and merge extra rows). `.env` now has real BC Self-Serve creds for local testing, not usable headlessly here. `parseAutoSubmitForm()` in http-scraper.js already has most of the hidden-field-harvesting logic to reuse.

...
- [ ] `.asc/workflow.json`'s `ship-mac` `export_mac` step uses `--pkg-path`, which this asc CLI version rejects (wants `--ipa-path` even for Mac, and requires the destination file end in `.ipa` even though Mac exports a `.pkg`) — workflow can't run end-to-end for Mac without manual intervention. Also `asc builds upload --pkg` requires `--build-number` explicitly — verify against the pkg's actual `Info.plist` (`pkgutil --expand-full` + `PlistBuddy -c "Print :CFBundleVersion"`) before passing it.
...
- [x] Avatar not persisting between sessions — root cause: `POST /api/avatar` deleted the old blob *before* saving the profile, so a failed save left `profile.avatarUrl` at a 404 and the `<img>` fell back to the letter icon permanently. Delete now happens after the save (`src/api.js`), plus a client self-heal (`web/unified.html`): a broken avatar `<img>` clears the cached URL and regenerates once.
...
- [x] Full payment history paywalled for the owner — `GET /api/stripe-status` (`src/api.js`) now returns `isPro: true` when the session's BCeID matches `BCEID_USERNAME`. That endpoint is the single gate every client reads, so the unlock covers web (and any future iOS gate) at once. `BCEID_USERNAME` was missing from the Vercel production env (the fix would have no-opped in prod) — added as a Sensitive Production var 2026-08-10.
- [x] Pay dates were WRONG — verified the hardcoded `payDates2026` table in `src/api.js` against BC's published cheque issue schedule: 5 of 12 months were off (Apr 23→22, Jul 23→29, Sep 24→23, Oct 28→21, Nov 25→18). Replaced with the transcribed official schedule in a new `src/pay-dates.js`; `tools/test-mobile-data.js` had its own drifted copy of the same wrong table, now imports production instead. Past the last published date it returns null (renders "--") rather than extrapolating a wrong benefits date. New `tools/test-pay-dates.js` (7 tests) wired into `npm test`; full suite green.

...
- [ ] Confirm Mac build 2be1761b reaches VALID (took noticeably longer to appear in `asc builds list` than iOS builds typically do — check again next session) — then `asc versions create --app 6782661988 --version 3.5.6` → `asc age-rating edit --app-info-id <new app-info id> --social-media false --social-media-age-restricted false` → `asc versions attach-build` → `asc validate` → `asc review submit --confirm` (same sequence used for iOS 3.5.7 and Sparkjar Mac this session).
- [ ] Also file upstream: `.asc/workflow.json`'s `ship-mac` `export_mac` step uses `--pkg-path`, which this asc CLI version rejects (wants `--ipa-path` even for Mac, and further requires the destination file end in `.ipa` even though Mac exports a `.pkg`) — the workflow can't currently run end-to-end for Mac without manual intervention.
